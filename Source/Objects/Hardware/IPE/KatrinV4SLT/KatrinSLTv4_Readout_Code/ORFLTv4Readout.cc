    ...
    //							if(diffsubsec<postTriggerTime) fprintf(stderr,"---=============break\n");
                            ...
                            for(eventchan=0;eventchan<kNumChan;eventchan++){
                                ...
                                if((channelMap & eventchanmask) && (triggerEnabledMask & eventchanmask)){
                                    ...
                                    uint32_t pagenr        = (f3 >> 24) & 0x3f;
                                   // uint32_t energy        = f3 & 0xfffff;
    //fprintf(stdout,"col:%i, rp: %i wp: %i , channelMap: %i pagenr:%i energy: %i \n\r",col+1,readptr,writeptr, channelMap,pagenr, energy); fflush(stdout);
    //fprintf(stdout,"-----------------------------                                  \n\r"); fflush(stdout);
        ...
        if(srack->theFlt[col]->isPresent()  && (forceFltReadoutFlag==kForceFltReadoutFlag)){
            ...
            // --- ENERGY MODE ------------------------------
            if((daqRunMode == kIpeFltV4_EnergyDaqMode) || (daqRunMode == kIpeFltV4_VetoEnergyDaqMode) || (daqRunMode == kIpeFltV4_BipolarEnergyDaqMode)){
            ...
            // --- HISTOGRAM MODE ------------------------------
            else if(fltRunMode == kIpeFltV4Katrin_Histo_Mode) {
                    hw4::FltKatrin *currentFlt = srack->theFlt[col];
                    uint32_t pageAB,oldpageAB;
                     if(runFlags & kFirstTimeFlag){// firstTime
                        //make some plausability checks
