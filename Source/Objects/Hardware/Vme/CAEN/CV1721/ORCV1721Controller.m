...
- (id) init
{
    self = [ super initWithWindowNibName: @"CV1721" ];
    return self;
}
...
- (void) registerNotificationObservers
{
    ...
    [notifyCenter addObserver : self
					 selector : @selector(selectedRegIndexChanged:)
						 name : ORCV1721SelectedRegIndexChanged
					   object : model];
	
    [notifyCenter addObserver : self
					 selector : @selector(selectedRegChannelChanged:)
						 name : ORCV1721SelectedChannelChanged
					   object : model];
	
    [notifyCenter addObserver : self
					 selector : @selector(writeValueChanged:)
						 name : ORCV1721WriteValueChanged
					   object : model];
	
    [notifyCenter addObserver : self
					 selector : @selector(thresholdChanged:)
						 name : ORCV1721ChnlThresholdChanged
					   object : model];
	
	[notifyCenter addObserver : self
					 selector : @selector(dacChanged:)
						 name : ORCV1721ChnlDacChanged
					   object : model];
	
	[notifyCenter addObserver : self
					 selector : @selector(overUnderChanged:)
						 name : ORCV1721OverUnderThresholdChanged
					   object : model];
	
    [notifyCenter addObserver : self
                     selector : @selector(channelConfigMaskChanged:)
                         name : ORCV1721ModelChannelConfigMaskChanged
					   object : model];
	
    [notifyCenter addObserver : self
                     selector : @selector(customSizeChanged:)
                         name : ORCV1721ModelCustomSizeChanged
					   object : model];

	[notifyCenter addObserver : self
			 selector : @selector(isCustomSizeChanged:)
			     name : ORCV1721ModelIsCustomSizeChanged
			   object : model];
	
	[notifyCenter addObserver : self
			 selector : @selector(isFixedSizeChanged:)
			     name : ORCV1721ModelIsFixedSizeChanged
			   object : model];
	
	[notifyCenter addObserver : self
                     selector : @selector(countAllTriggersChanged:)
                         name : ORCV1721ModelCountAllTriggersChanged
					   object : model];
	
    [notifyCenter addObserver : self
                     selector : @selector(acquisitionModeChanged:)
                         name : ORCV1721ModelAcquisitionModeChanged
					   object : model];
	
    [notifyCenter addObserver : self
                     selector : @selector(coincidenceLevelChanged:)
                         name : ORCV1721ModelCoincidenceLevelChanged
					   object : model];
	
    [notifyCenter addObserver : self
                     selector : @selector(triggerSourceMaskChanged:)
                         name : ORCV1721ModelTriggerSourceMaskChanged
					   object : model];
	[notifyCenter addObserver : self
			 selector : @selector(triggerOutMaskChanged:)
			     name : ORCV1721ModelTriggerOutMaskChanged
			   object : model];

	[notifyCenter addObserver : self
			 selector : @selector(fpIOControlChanged:)
			     name : ORCV1721ModelFrontPanelControlMaskChanged
			   object : model];
	
    [notifyCenter addObserver : self
                     selector : @selector(postTriggerSettingChanged:)
                         name : ORCV1721ModelPostTriggerSettingChanged
					   object : model];
	
    [notifyCenter addObserver : self
                     selector : @selector(enabledMaskChanged:)
                         name : ORCV1721ModelEnabledMaskChanged
					   object : model];
	
	[notifyCenter addObserver : self
					 selector : @selector(basicLockChanged:)
						 name : ORCV1721BasicLock
					   object : nil];
	
	[notifyCenter addObserver : self
					 selector : @selector(settingsLockChanged:)
						 name : ORCV1721SettingsLock
					   object : nil];
    ...
    [notifyCenter addObserver : self
                     selector : @selector(setBufferStateLabel)
                         name : ORCV1721ModelBufferCheckChanged
                       object : model];
	
    [notifyCenter addObserver : self
                     selector : @selector(eventSizeChanged:)
                         name : ORCV1721ModelEventSizeChanged
						object: model];
...
- (void) checkGlobalSecurity
{
    ...
    [gSecurity setLock:ORCV1721BasicLock to:secure];
    [basicLockButton setEnabled:secure];
    [gSecurity setLock:ORCV1721SettingsLock to:secure];
...
- (void) selectedRegIndexChanged:(NSNotification*) aNotification
{
	...
	BOOL lockedOrRunningMaintenance = [gSecurity runInProgressButNotType:eMaintenanceRunType orIsLocked:ORCV1721BasicLock];
...
- (void) thresholdChanged:(NSNotification*) aNotification
{
	...
	if(aNotification){
		int chnl = [[[aNotification userInfo] objectForKey:ORCV1721Chnl] intValue];
...
{	
    ...
    BOOL locked						= [gSecurity isLocked:ORCV1721BasicLock];
    BOOL lockedOrRunningMaintenance = [gSecurity runInProgressButNotType:eMaintenanceRunType orIsLocked:ORCV1721BasicLock];
    ...
    if(lockedOrRunningMaintenance){
		if(runInProgress && ![gSecurity isLocked:ORCV1721BasicLock])s = @"Not in Maintenance Run.";
    }
...
{	
    BOOL runInProgress				= [gOrcaGlobals runInProgress];
    BOOL locked						= [gSecurity isLocked:ORCV1721SettingsLock];
    BOOL lockedOrRunningMaintenance = [gSecurity runInProgressButNotType:eMaintenanceRunType orIsLocked:ORCV1721SettingsLock];
...
{
    [gSecurity tryToSetLock:ORCV1721BasicLock to:[sender intValue] forWindow:[self window]];
}

- (IBAction) settingsLockAction:(id)sender
{
    [gSecurity tryToSetLock:ORCV1721SettingsLock to:[sender intValue] forWindow:[self window]];
}
