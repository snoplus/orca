...
- (void) awakeFromNib
{
    ...
    NSString* key = [NSString stringWithFormat: @"orca.HVcRIO%lu.selectedtab",[model uniqueIdNumber]];
...
- (void) registerNotificationObservers
{
    ...
    [notifyCenter addObserver : self
                     selector : @selector(pollingStateChanged:)
                         name : ORHVcRIOModelPollingStateChanged
						object: model];
    ...
    [notifyCenter addObserver : self
                     selector : @selector(adcChanged:)
                         name : ORHVcRIOModelAdcChanged
                       object : nil];
			
    [notifyCenter addObserver : self
                     selector : @selector(gainsChanged:)
                         name : ORHVcRIOModelGainsChanged
						object: model];

    [notifyCenter addObserver : self
                     selector : @selector(gainsReadBackChanged:)
                         name : ORHVcRIOModelGainsReadBackChanged
						object: model];

    
    [notifyCenter addObserver : self
                     selector : @selector(logToFileChanged:)
                         name : ORHVcRIOModelLogToFileChanged
						object: model];
	
    [notifyCenter addObserver : self
                     selector : @selector(logFileChanged:)
                         name : ORHVcRIOModelLogFileChanged
						object: model];
    ...
    [notifyCenter addObserver : self
                     selector : @selector(gainDisplayTypeChanged:)
                         name : ORHVcRIOModelGainDisplayTypeChanged
						object: model];

    [notifyCenter addObserver : self
                     selector : @selector(processLimitsChanged:)
                         name : ORHVcRIOModelProcessLimitsChanged
						object: model];

    [notifyCenter addObserver : self
                     selector : @selector(lcmChanged:)
                         name : ORHVcRIOModelLcmChanged
						object: model];
    
    [notifyCenter addObserver : self
                     selector : @selector(workingOnGainChanged:)
                         name : ORHVcRIOModelWorkingOnGainChanged
						object: model];

    [notifyCenter addObserver : self
                     selector : @selector(setGainsResultChanged:)
                         name : ORHVcRIOModelSetGainsResultChanged
						object: model];

    [notifyCenter addObserver : self
                     selector : @selector(preAmpSelectionChanged:)
                         name : ORHVcRIOModelPreAmpSelectionChanged
						object: model];

    [notifyCenter addObserver : self
                     selector : @selector(channelSelectionChanged:)
                         name : ORHVcRIOModelChannelSelectionChanged
						object: model];

    [notifyCenter addObserver : self
                     selector : @selector(lcmEnabledChanged:)
                         name : ORHVcRIOModelLcmEnabledChanged
						object: model];

    [notifyCenter addObserver : self
                     selector : @selector(vetoConditionChanged:)
                         name : ORHVcRIOModelVetoChanged
						object: model];
...
- (void) lockChanged:(NSNotification*)aNotification
{
    ...
    //BOOL lockedOrRunningMaintenance = [gSecurity runInProgressButNotType:eMaintenanceRunType orIsLocked:ORHVcRIOLock];
...
#pragma mark •••Table Data Source
...
{	
    ...
    [[NSUserDefaults standardUserDefaults] setInteger:index forKey:[NSString stringWithFormat:@"orca.HVcRIO%lu.selectedtab",[model uniqueIdNumber]]];
...
#pragma  mark •••Delegate Responsiblities
