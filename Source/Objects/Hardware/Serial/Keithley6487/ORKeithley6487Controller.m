...
#import "ORKeithley6487Controller.h"
...
@interface ORKeithley6487Controller (private)
- (void) populatePortListPopup;
@end

@implementation ORKeithley6487Controller

#pragma mark ***Initialization

- (id) init
{
	self = [super initWithWindowNibName:@"Keithley6487"];
	return self;
}
...
- (void) registerNotificationObservers
{
    ...
    [notifyCenter addObserver : self
                     selector : @selector(lockChanged:)
                         name : ORKeithley6487Lock
                        object: nil];

    [notifyCenter addObserver : self
                     selector : @selector(portNameChanged:)
                         name : ORKeithley6487ModelPortNameChanged
                        object: nil];
    ...
    [notifyCenter addObserver : self
                     selector : @selector(currentChanged:)
                         name : ORKeithley6487CurrentChanged
                       object : nil];

    [notifyCenter addObserver : self
                     selector : @selector(pollTimeChanged:)
                         name : ORKeithley6487ModelPollTimeChanged
                       object : nil];

    [notifyCenter addObserver : self
                     selector : @selector(shipCurrentChanged:)
                         name : ORKeithley6487ModelShipCurrentChanged
						object: model];
...
- (void) checkGlobalSecurity
{
    ...
    [gSecurity setLock:ORKeithley6487Lock to:secure];
...
- (void) lockChanged:(NSNotification*)aNotification
{
    ...
    BOOL lockedOrRunningMaintenance = [gSecurity runInProgressButNotType:eMaintenanceRunType orIsLocked:ORKeithley6487Lock];
    BOOL locked = [gSecurity isLocked:ORKeithley6487Lock];
    ...
    if(lockedOrRunningMaintenance){
        if(runInProgress && ![gSecurity isLocked:ORKeithley6487Lock])s = @"Not in Maintenance Run.";
    }
...
{
    [gSecurity tryToSetLock:ORKeithley6487Lock to:[sender intValue] forWindow:[self window]];
}
...
@implementation ORKeithley6487Controller (private)
