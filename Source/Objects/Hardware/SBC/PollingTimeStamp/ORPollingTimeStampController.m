//
//  ORPollingTimeStampController.m
//  Orca
//
...
#import "ORPollingTimeStampController.h"
...
@implementation ORPollingTimeStampController

-(id)init
{
    self = [super initWithWindowNibName:@"PollingTimeStamp"];
...
{
    [super awakeFromNib];
    NSString* key = [NSString stringWithFormat: @"orca.ORPollingTimeStamp%d.selectedtab",[model slot]];
...
- (void) registerNotificationObservers
{
    ...
    [notifyCenter addObserver : self
					 selector : @selector(thresholdArrayChanged:)
						 name : ORPollingTimeStampThresholdArrayChangedNotification
					   object : model];
	
    [notifyCenter addObserver : self
					 selector : @selector(gainArrayChanged:)
						 name : ORPollingTimeStampGainArrayChangedNotification
					   object : model];
    ...
    [notifyCenter addObserver : self
					 selector : @selector(thresholdChanged:)
						 name : ORPollingTimeStampThresholdChangedNotification
					   object : model];
	
	
    [notifyCenter addObserver : self
					 selector : @selector(gainChanged:)
						 name : ORPollingTimeStampGainChangedNotification
					   object : model];
	
	
    [notifyCenter addObserver : self
					 selector : @selector(continousChanged:)
						 name : ORPollingTimeStampContinousChangedNotification
					   object : model];
	
    [notifyCenter addObserver : self
					 selector : @selector(scalersEnabledChanged:)
						 name : ORPollingTimeStampScalersEnabledChangedNotification
					   object : model];
	
    [notifyCenter addObserver : self
					 selector : @selector(multiBoardEnabledChanged:)
						 name : ORPollingTimeStampMultiBoardEnabledChangedNotification
					   object : model];
	
    [notifyCenter addObserver : self
					 selector : @selector(scalerMaskChanged:)
						 name : ORPollingTimeStampScalerMaskChangedNotification
					   object : model];
	
    [notifyCenter addObserver : self
					 selector : @selector(onlineMaskChanged:)
						 name : ORPollingTimeStampOnlineMaskChangedNotification
					   object : model];
	
	
    [notifyCenter addObserver : self
					 selector : @selector(scanStartChanged:)
						 name : ORPollingTimeStampScanStartChangedNotification
					   object : model];
	
    [notifyCenter addObserver : self
					 selector : @selector(scanDeltaChanged:)
						 name : ORPollingTimeStampScanDeltaChangedNotification
					   object : model];
	
    [notifyCenter addObserver : self
					 selector : @selector(scanNumberChanged:)
						 name : ORPollingTimeStampScanNumChangedNotification
					   object : model];
    ...
    [notifyCenter addObserver : self
					 selector : @selector(rateGroupChanged:)
						 name : ORPollingTimeStampRateGroupChangedNotification
					   object : model];
    ...
    [notifyCenter addObserver : self
					 selector : @selector(displayRawChanged:)
						 name : ORPollingTimeStampDisplayRawChangedNotification
					   object : model];
    ...
    [notifyCenter addObserver : self
					 selector : @selector(settingsLockChanged:)
						 name : ORPollingTimeStampSettingsLock
						object: nil];
    ...
    [notifyCenter addObserver : self
                     selector : @selector(shipTimeStampChanged:)
                         name : ORPollingTimeStampModelShipTimeStampChanged
						object: model];
...
- (void) checkGlobalSecurity
{
    ...
    [gSecurity setLock:ORPollingTimeStampSettingsLock to:secure];
...
- (void) settingsLockChanged:(NSNotification*)aNotification
{
    ...
    BOOL lockedOrRunningMaintenance = [gSecurity runInProgressButNotType:eMaintenanceRunType orIsLocked:ORPollingTimeStampSettingsLock];
    BOOL locked = [gSecurity isLocked:ORPollingTimeStampSettingsLock];
    ...
    if(lockedOrRunningMaintenance){
		if(runInProgress && ![gSecurity isLocked:ORPollingTimeStampSettingsLock])s = @"Not in Maintenance Run.";
    }
...
- (void) slotChanged:(NSNotification*)aNotification
{
	...
	[[self window] setTitle:[NSString stringWithFormat:@"PollingTimeStamp Card (Crate %d,Slot %d)",[model crateNumber],[model slot]]];
}
...
{
	short chan;
	for(chan=0;chan<kNumPollingTimeStampChannels;chan++){
...
{
	int chan = [[[aNotification userInfo] objectForKey:ORPollingTimeStampChan] intValue];
	[self updateStepper:[thresholdSteppers cellWithTag:chan] setting:[model threshold:chan]];
	if([model displayRaw]){
		[[thresholdTextFields cellWithTag:chan] setIntValue: [model threshold:chan]];
	}
...
- (void) scalerMaskChanged:(NSNotification*)aNotification
{
	...
	for(i=0;i<kNumPollingTimeStampChannels;i++){
...
{
    [gSecurity tryToSetLock:ORPollingTimeStampSettingsLock to:[sender intValue] forWindow:[self window]];
}
...
-(IBAction)initBoard:(id)sender
{
    @try {
		...
		NSLog(@"Initialized PollingTimeStamp (Slot %d <%p>)\n",[model slot],[model baseAddress]);
        
    }
	@catch(NSException* localException) {
        NSLog(@"Reset and Init of PollingTimeStamp FAILED.\n");
        NSRunAlertPanel([localException name], @"%@\nFailed PollingTimeStamp Reset and Init", @"OK", nil, nil,
                        localException);
    }
}
...
- (IBAction) report:(id)sender
{
	...
	@catch(NSException* localException) {
		NSLog(@"Read Gain of PollingTimeStamp FAILED.\n");
...
- (IBAction) readScalers:(id)sender
{
	@try {
		[self endEditing];
		short chan;
		NSLog(@"Scalers for PollingTimeStamp in Slot %d  <0x%x> enabled: %@\n",[model slot],[model baseAddress],[model scalersEnabled]?@"YES":@"NO");
	...
	@catch(NSException* localException) {
		NSLog(@"Read Scalers of PollingTimeStamp FAILED.\n");
...
{
    [self endEditing];
    [model scanForPollingTimeStamps];
...
-(IBAction)probeBoard:(id)sender
{
	...
	@catch(NSException* localException) {
        NSLog(@"Probe PollingTimeStamp Board FAILED.\n");
