...
@implementation SNOCaenWaveformDecoder
    ...
    while (eventLength > 3) { //make sure at least the CAEN header is there
        ...
        for(j=0;j<numChans;j++){
            ...
            if(getRatesFromDecodeStage && !skipRateCounts){
                ...
                SNOCaenModel* obj = [actualCards objectForKey:aKey];
                if(!obj){
                    NSArray* listOfCards = [[(ORAppDelegate*)[NSApp delegate] document] collectObjectsOfClass:NSClassFromString(@"SNOCaenModel")];
