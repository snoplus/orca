...
- (ORPQResult *) listDBs
{
    return [self queryString:@"SELECT datname FROM pg_database WHERE datistemplate = false;"];
}


- (ORPQResult*) listTables
{
    return [self queryString:@"SELECT table_schema,table_name FROM information_schema.tables ORDER BY table_schema,table_name;"];
}
