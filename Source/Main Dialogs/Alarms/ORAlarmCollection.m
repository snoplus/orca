...
- (void) eMailThread
{
	...
	NSAutoreleasePool* xpool = [[NSAutoreleasePool alloc] init];
	BOOL configured;
		...
		while(eMailThreadRunning){
			...
			if(subscribedToAtleastOne){
				...
				if([content length] != 0){
					...
					BOOL result = NO;
					@synchronized([NSApp delegate]){
						result = [NSMailDelivery deliverMessage:[emailHeader stringByAppendingString:content] 
														subject:@"Orca Alarms"
															 to: mailAddress];
						result = 1;
					}
					if(result){
						NSLog(@"Alarm report mailed to: %@\n",mailAddress);
						[alarms removeObjectsInArray: alarmsSent];
					}
					else {
						NSLogColor([NSColor redColor], @"Alarm report FAILED to be mailed to: %@\n",mailAddress);
					}
				}
			}
			[eMailLock unlock];
